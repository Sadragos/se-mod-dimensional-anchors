...
{
    [MyEntityComponentDescriptor(typeof(MyObjectBuilder_CargoContainer), true, "AdminConsumableGenerator")]
    public class ConsumableGen : MyGameLogicComponent
    {
        ...
        public override void UpdateBeforeSimulation100()
        {
            ...
            if (m_generator.IsWorking)
            {
                ...
                if(num < 2) {
                    inventory.AddItems(amount, new MyObjectBuilder_ConsumableItem() { SubtypeName = "Medkit" });
                    terminalBlock.RefreshCustomInfo();
                } else if(num < 4) {
                    inventory.AddItems(amount, new MyObjectBuilder_ConsumableItem() { SubtypeName = "Powerkit" });
                    terminalBlock.RefreshCustomInfo();
                }
